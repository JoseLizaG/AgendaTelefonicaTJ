...
namespace PredicacionPorTelefono.Controllers
{
    public class CasasController : Controller
    {
        ...
        // GET: Casas/Notas/5
        public async Task<ActionResult> Notas(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Notas([Bind(Include = "Id,TerritorioId,Tipo,Calle,Numero,DepInt,Telefono1,Telefono2,Telefono3,Abonado,Distrito,Observaciones")] Casa casa)
        {
            if (ModelState.IsValid)
            {
                db.Entry(casa).State = EntityState.Modified;
